...
//Creating new object for PHPExcel
    ...
    $objPHPexcel = PHPExcel_IOFactory::load('C:/xampp/htdocs/chuechuenge/TopItineraryReport.xlsx');
    $objPHPexcel->getActiveSheet()->getCell('J6')->setValue("$reporttime");
	$SDate = $_POST['SDate'];
	$SDate1 = $_POST['SDate1'];
    
    $query = " SELECT * FROM `Peacock`.`Booking` WHERE `Booking`.`SDate` between '$SDate' AND '$SDate1' ORDER BY ItineraryPrice ";
       ...
       //$row = mysql_fetch_array($result);
        while($row = mysql_fetch_array($result))
        {
            ...
            $objPHPexcel->getActiveSheet()->getCell('D' . $i)->setValue("$row[ItineraryName]");
            $objPHPexcel->getActiveSheet()->getCell('E' . $i)->setValue("$row[ItineraryPrice]");
        ...
        $objWriter->save('TopItineraryReport.xls');
        echo"<script type='text/javascript'>window.location='TopItineraryReport.xls'; </script>";
