...
<li><a href="index.php">Logout</a></li>
	...
	<div id="main112">
	<center>
	<h1>  Yearly Top Rented Ship Report Successful</h1>
	</center></div>
	</body>
</html>
<?php
...
//Creating new object for PHPExcel
    ...
    $objPHPexcel = PHPExcel_IOFactory::load('C:/xampp/htdocs/chuechuenge/YearlyTopRentedReport.xlsx');
    $objPHPexcel->getActiveSheet()->getCell('J6')->setValue("$reporttime");
	$SDate = $_POST['SDate'];
	$SDate1 = $_POST['SDate1'];
    
    $query = " SELECT * FROM `Peacock`.`Booking` WHERE `Booking`.`SDate` between '$SDate' AND '$SDate1' ORDER BY VesselPrice ";
       ...
       //$row = mysql_fetch_array($result);
        while($row = mysql_fetch_array($result))
        {
            ...
            $objPHPexcel->getActiveSheet()->getCell('D' . $i)->setValue("$row[VesselName]");
            $objPHPexcel->getActiveSheet()->getCell('E' . $i)->setValue("$row[VesselPrice]");
            $objPHPexcel->getActiveSheet()->getCell('F' . $i)->setValue("$row[TotalAmount]");
        ...
        $objWriter->save('YearlyTopRentedReport.xls');
        echo"<script type='text/javascript'>window.location='YearlyTopRentedReport.xls'; </script>";
