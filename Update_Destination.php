...
$result = mysql_query ("SELECT * FROM `peacock`.`destination` WHERE `destination`.`DestinationID` = '$DestinationID'");
$row = mysql_fetch_assoc($result);
?>
<form method="post"  action="EditDestination.php" enctype="multipart/form-data">
<input type = "hidden" name = "DestinationID" value = "<?php echo $row['DestinationID'] ?>"> 
<center><table cellspacing="20px">
<caption><h1> Editing Destination </h1></caption>
<tr>
<td><label >Destination Name </label></td>
<td><input type="text" id = "DName" name = "DName" value= "<?php echo $row['DestinationName'] ?>"></td></tr>
<tr>
<td><label > Destination Image </label></td>
<td><img src="image/<?php echo $row['DestinationImage'] ?>" width="100px" height="100px" >
<input type="file" name = "DImage" id = "DImage"></td>
</tr>
